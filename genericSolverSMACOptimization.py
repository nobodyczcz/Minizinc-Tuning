...
class CPLEX(GenericSolverExpansion):
    ...
    def pSMAC_wrapper_generator(self):
        ...
        for i in range(self.nSMAC):
            writeToFile = []
            writeToFile.append('import inspect, os.path, sys')
            writeToFile.append('filename = inspect.getframeinfo(inspect.currentframe()).filename')
            writeToFile.append('path = os.path.dirname(os.path.abspath(filename))')
            writeToFile.append('sys.path.insert(0, path)')
