...
def main():
    ...
    if args.instances_file != None:
        ...
        instanceList = process_instance(ins_path=args.instances_file)
    else:
        if args.instances == None:
            ...
            ins_list = re.findall('([^\s\.]+\.mzn(?:\s[^\.]+\.dzn)+)+', ' '.join(args.instances))
            # Example commandline input of instances:
            # model1.mzn data1.dzn data2.dzn model2.mzn data3.dzn model3.mzn data4.dzn
            # Expected output to list after regex searching:
            # ['model1.mzn data1.dzn data2.dzn', 'model2.mzn data3.dzn', 'model3.mzn data4.dzn']
            # call instances-process(instancesList=args.instances)
            #instanceList = process_instance(ins_list=['mapping.mzn full2x2_mp3.dzn'])
            instanceList = process_instance(ins_list)
    ...
    (n_SMAC, n_MINIZINC) = process_thread(n_thread=args.p)
    ...
    try:
        ...
        #if args.cut == None:
        cutoff_time = instance_runtime(instanceList, n_MINIZINC, param_config_space, solverFlag, args.cplex_dll) * 5
        print('cut off time:',cutoff_time)     
        print("Generating wrapper files for SMAC")

        pSMAC_wrapper_generator(n_SMAC, n_MINIZINC, args.cplex_dll)
        
        print("Generating scenario files for SMAC")
        pSMAC_scenario_generator(n_SMAC, param_config_space, cutoff_time, args.t)
        #print("Generating command line for SMAC")
        #commandline_psamc = psmac_args(n_SMAC, solverFlag, True)
        print("Call pSAMC")
        pid = psamc_exe(n_SMAC, solverFlag, True)
    except KeyboardInterrupt:
        print("\n\nKeyboardInterrupt has been caught. Cleaning up...")
        for process in psutil.process_iter():      
            if set(['python', '--scenario']).issubset(set(process.cmdline())):
        ...
        print("Success.")
        benchmark_main(instanceList, solverFlag, n_MINIZINC, args.cplex_dll, 5)
        remove_tmp_files(instanceList)        
    '''
